              ...
              child: BlocBuilder<DriverStoreCubit, DriverStoreState>(
                builder: (context, state) {
                  if (state is DriverStoreLoading) {
                    ...
                    if (packages.isEmpty) {
                      return Center(child: Text("لا توجد باقات متاحة حالياً"));
                    }
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  Text(
                    "$points نقطة",
                    style: TextStyle(fontSize: 24.sp, fontWeight: FontWeight.bold, color: AppColors.primaryColor),
                  ),
                ],
              ),
            ],
          ...
          children: [
                ...
                if (package.image != null && package.image!.isNotEmpty)
                  ClipRRect(
                    ...
                    child: Image.network(
                      ...
                      height: 60.r,
                      fit: BoxFit.cover,
                      errorBuilder: (_, __, ___) => Icon(Icons.card_giftcard, size: 40.r, color: Colors.grey),
                    ),
                  )
                else
                  Container(
                    ...
                    child: Icon(Icons.card_giftcard, size: 30.r, color: Colors.grey[600]),
                  ),
                SizedBox(width: 12.w),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 4.h),
                      Text(
                        "خصم عمولة: %${package.discountPercentage}",
                        style: TextStyle(fontSize: 14.sp, color: Colors.green, fontWeight: FontWeight.w600),
                      ),
                      Text(
                        "صالحة لمدة ${package.validDays} يوم",
            ...
            Row(
              children: [
                if (package.pricePoints != null && package.pricePoints! > 0)
                  Expanded(
                    child: ElevatedButton.icon(
                      ...
                      icon: Icon(Icons.stars, color: Colors.orange, size: 18.r),
                      label: Text("${package.pricePoints} نقطة", style: TextStyle(fontSize: 12.sp, color: Colors.black)),
                ...
                if (package.pricePoints != null && package.pricePoints! > 0 && package.priceMoney != null && package.priceMoney! > 0)
                  SizedBox(width: 8.w),
                if (package.priceMoney != null && package.priceMoney! > 0)
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: () => _buy(context, 'wallet'),
                      icon: Icon(Icons.account_balance_wallet, color: Colors.white, size: 18.r),
                      label: Text("${package.priceMoney} ج.م", style: TextStyle(fontSize: 12.sp)),
                      style: ElevatedButton.styleFrom(
  ...
  void _buy(BuildContext context, String method) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text("هل أنت متأكد من شراء الباقة باستخدام ${method == 'points' ? 'النقاط' : 'المحفظة'}؟"),
        actions: [
          TextButton(
